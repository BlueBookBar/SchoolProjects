...
public class Student extends Thread {
	...
	public static volatile int Seats=Main.numSeat;//Number of seats available
	...
	public static LinkedList<Group> Groups = new LinkedList<Group>();//Will keep track of groups
	public static int Numa=0;//Number of waiting threads to be grouped
	public static Group temp= new Group();//Group to be inserted into LL
	...
	public void Grouping(){
		while(true){
			...
			try {//if there are no more groups available 
				if(Completed  >= Main.maxGroupSize){
		...
		synchronized(waitForOthers){
			if(Completed  < Main.maxGroupSize){//For complete groups
				//wait method
				Numa++;
				if(Numa == 3){
					temp = new Group();
	...
	public void Marching(){
		try {
			synchronized(Clock.paradeMon){
				if(Main.Timing.isAlive()){
					Clock.paradeMon.wait();
					attendedParades =attendedParades+ " "+Clock.paradeTitle+", ";
					lastAttended= true;
					sleep(Clock.randomTimeParade);
				}
			}
		}
		catch (InterruptedException e) {
	...
	public void Watching(){
		try {
			synchronized(Clock.showMon){

				if(Main.Timing.isAlive()){
					if(Clock.showOver || (Seats== 0 )){//If the shows are over or Seats taken, do nothing
						lastAttended=false;
					}
					else{
						...
						Seats--;
						Clock.showMon.wait();
						attendedShows =attendedShows+ " "+Clock.showTitle+", ";
						lastAttended=true;
						sleep(Clock.randomTimeShow);
						Seats++;
					}
				}
			}
